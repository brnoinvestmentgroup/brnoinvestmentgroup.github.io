   ...
   'use strict';
	...
	$(document).scroll(function() { 
		...
		if($('section.home').length && $(this).scrollTop() < 180) {
			$('header').stop().animate({ backgroundColor: 'rgba(17,17,17,0.5)' }, 'fast');
		} else {
	...
	/*Clients
	$(".client").hover(function(e){
		$(this).find("img").stop().animate({ opacity: 1 }, 'slow');
	}, function(){ 
		$(this).find("img").stop().animate({ opacity: 0.4 }, 'slow');
	});

	$(".clients .grid:last-child .client").each(function() {
	...
	$(".project").hover(function(e){
		$(this).find(".thumb img").addClass("active");
		$(this).find(".info").addClass("active");
	}, function(){
		$(this).find(".info").removeClass("active");
		$(this).find(".thumb img").removeClass("active");
	});
	...
	/*Services
	$(".services .filter li").hover(function(e){
		$(this).find("span").stop().animate({ opacity: 1 }, 'fast');
	}, function(){ 
		$(".services .filter li").find("span").stop().animate({ opacity: 0.5 }, 'fast');
	});
	...
	/*Facts
	$(".facts .fact").hover(function(e){
		$(".facts .fact").stop().animate({ opacity: 0.5 }, 'slow');
		$(this).stop().animate({ opacity: 1 }, 'slow');
	}, function(){ 
		$(".facts .fact").stop().animate({ opacity: 1 }, 'slow');
	});
...
$(window).load(function() {
	...
	if($('section.home').length && $(this).scrollTop() < 180) {
		$('header').stop().animate({ backgroundColor: 'rgba(17,17,17,0.3)' }, 'fast');
	} else {
		$('header').stop().animate({ backgroundColor: 'rgba(17,17,17,1)' }, 'fast');
	}
    ...
    $('#clients').waypoint(function() {
        setTimeout(function(){$('#clients .grid').addClass('animated fadeInDown')},0);
    }, { offset: '50' });

    $('#quotes').waypoint(function() {
        setTimeout(function(){$('#quotes .slider').addClass('animated fadeInDown')},0);
    }, { offset: '50' });

    $('#facts').waypoint(function() {
        setTimeout(function(){$('#facts .fact').addClass('animated fadeInDown')},0);
    }, { offset: '50' });

    $('#services').waypoint(function() {
        setTimeout(function(){$('#services .filter li').addClass('animated fadeInDown')},0);
        setTimeout(function(){$('#services .service').addClass('animated fadeInUp')},0);
    }, { offset: '50' });
    ...
    $('#contact').waypoint(function() {
        setTimeout(function(){$('#contact .two-thirds').addClass('animated fadeInLeft')},0);
        setTimeout(function(){$('#contact .one-third').addClass('animated fadeInRight')},0);
    }, { offset: '50' });
    //Animations
